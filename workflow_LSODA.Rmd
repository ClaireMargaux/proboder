...
initial_params <-
    initialization(model = model, obs = obs_to_use,
                   ...
                   noise_X = sqrt(noise_obs), noise_U = 0.5,
...
if (TRUE) { # set TRUE to perform random search
  ...
  seq_l <- seq(1, 100, by = 5)
  seq_wiener_X <- seq(1, 1000, by = 50)
  seq_wiener_U <- seq(0.001, 2, by = 0.1)
  ...
  plots_scores <- plot_scores(results_summary, aggregate = TRUE)
...
if (TRUE) { # set TRUE to perform random search
  ...
  results_summary <- 
    run_random_search(
      ...
      noise_obs = noise_obs, 
      lambda = lambda, gamma = gamma, eta = eta, pop = pop, 
      beta0 = beta0, jit_X = jit_X, jit_U = jit_U,
      seed = seed, num_param_sets = num_param_sets, 
      seq_l = seq_l, 
      seq_wiener_X = seq_wiener_X, 
      seq_wiener_U = seq_wiener_U, 
